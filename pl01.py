...
st.markdown("Explorar dados históricos da Fórmula 1 com **Pandas + Streamlit + Plotly**")
...
piloto_filter = st.sidebar.multiselect(
    "Selecionar piloto(s)",
    pilotos_list,
    pilotos_list
)

equipas_list = constructors['constructorRef'].unique()

equipa_filter = st.sidebar.multiselect(
    "Selecionar equipa(s)",
    equipas_list,
    equipas_list
)
...
# Função garantir valores numérico
...
with tabs[0]:
    ...
    points_df = df.groupby('driverRef')['points'].sum() \
        .sort_values(ascending=False).head(10) \
        .to_frame("Pontos").reset_index()
    ...
    col2.dataframe(
        points_df.style.format({'Pontos': "{:.0f}"})
    )
...
with tabs[1]:
    ...
    col2.dataframe(
        wins_df.style.format({'Vitórias': "{:.0f}"})
    )
...
with tabs[2]:
    ...
    points_team = df_team.groupby('constructorRef')['points'] \
    ...
    col2.dataframe(
        points_team.style.format({'Pontos': "{:.0f}"})
    )
...
with tabs[3]:
    ...
    abandonos_circ = abandonos.groupby('circuitRef') \
        .size().sort_values(ascending=False) \
        .head(10).to_frame("Abandonos").reset_index()

    abandonos_circ["Abandonos"] = pd.to_numeric(abandonos_circ["Abandonos"])
    ...
    col2.dataframe(
        abandonos_circ.style.format({"Abandonos": "{:.0f}"})
    )
...
with tabs[4]:
    ...
    st.subheader("Melhor consistência")

    consistency = df.groupby('driverRef')['points'] \
        .median().sort_values(ascending=False) \
        .head(10).to_frame("Mediana Pontos").reset_index()
...
with tabs[5]:
    ...
    nationality = wins_df.groupby('nationality') \
        .size().sort_values(ascending=False) \
        .head(10).to_frame("Vitórias").reset_index()
